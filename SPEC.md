...
**細分化アルゴリズム（`subdivideGeoByEdge`）と T字接合対策**

ゾーン内の三角形のみを細分化すると、ゾーン内外の境界で頂点が共有されない **T字接合（T-junction）** が発生し、シェル生成時のステッチ（`weldGeo`）が正常に機能せず穴・クラックが生じる。

これを防ぐため、2フェーズ処理を採用する。

```
Phase 1: 細分化対象の三角形をすべてスキャンし、その3辺すべての中点を
         edgeMidMap（辺キー → 中点座標）に登録する。
         辺キーは両端点を辞書順ソートして方向非依存にする。

Phase 2: 全三角形を処理
  - ゾーン内・長辺あり         → 通常の4分割
  - 隣接分割辺なし（n=0）       → そのまま通過
  - 隣接分割辺が1辺（n=1）      → 2分割（T字接合解消）
  - 隣接分割辺が2辺（n=2）      → 3分割（T字接合解消）
  - 隣接分割辺が3辺（n=3）      → 4分割（境界三角形を完全に分割）
```

これにより、ゾーン内外の境界で頂点座標が完全に一致し、T字接合が解消される。
